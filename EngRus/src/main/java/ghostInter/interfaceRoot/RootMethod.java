...
//                my_word_ru[i].setCursor(Cursor.HAND);
    ...
    default void counter() {
        ...
        resultExam.setOnAction(event -> sow());
...
//        counter.setStyle("-fx-border-color: RED");
    ...
    default void statisticsWindow(){
        ...
        HBox hBox = new HBox();
        Label dateTime = new Label("TEst");
        Label returnYES = new Label("Test2");
        Label returnNO = new Label("Test3");
        hBox.getChildren().addAll(dateTime, returnYES, returnNO);
        VBox vBox = new VBox();
        ...
        group.getChildren().addAll(vBox, addCounter);
...
//        System.out.println(counterNO.getText());
    ...
    default void sow(){
        // TODO разбить запуск и инициализацию модального окна статистики на два метода!!!
        win.show();
    }
